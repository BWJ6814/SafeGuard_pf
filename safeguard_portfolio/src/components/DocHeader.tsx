...
export function DocHeader({
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-30 border-b border-slate-200/90 bg-white/95 shadow-[0_1px_0_0_rgba(15,23,42,0.06)] backdrop-blur-md supports-[backdrop-filter]:bg-white/90 lg:left-[20%]">
      <div className="flex min-h-14 items-center gap-3 px-3 py-2.5 sm:px-4 lg:pl-6 lg:pr-5">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-3 sm:items-center">
          <div
            className={`mt-0.5 hidden h-10 w-10 shrink-0 items-center justify-center rounded-xl ring-1 ring-inset ring-slate-200 sm:flex ${
              isIntro
          ...
          <div className="min-w-0 flex-1">
            {isIntro ? (
              ...
              <h1 className="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1 text-left text-lg font-bold leading-snug sm:text-xl">
                <span className="text-slate-900">{labels.category}</span>
                <span
